...
if ( $sent_to_admin ) {
	...
	$after  = '</h2>';
	/* translators: %s: Order ID. */
	echo wp_kses_post( $before . '<span class="yaymail-underline">' . sprintf( __( '[Order #%s]', 'woocommerce' ) . '</span> (<span><time datetime="%s">%s</time></span>)', $order->get_order_number(), $order->get_date_created()->format( 'c' ), wc_format_datetime( $order->get_date_created() ) ) . $after );
}
