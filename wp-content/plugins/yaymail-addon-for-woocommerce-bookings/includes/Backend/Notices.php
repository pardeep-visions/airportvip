...
class Notices {
	public static function initialize() {
		add_action( 'after_plugin_row_' . YAYMAIL_ADDON_WOO_BOOKING_PLUGIN_BASENAME, array( __CLASS__, 'install_plugin_notice' ), 10, 2 );
	}
	...
	public static function install_plugin_notice( $plugin_file, $plugin_data ) {
		...
		?>
		<script>
		var plugin_row_element = document.querySelector('tr[data-plugin="<?php echo esc_js( YAYMAIL_ADDON_WOO_BOOKING_PLUGIN_BASENAME ); ?>"]');
		...
		$notice_addon .= sprintf( esc_html__( 'In order to customize templates of %1$s, please install %2$s first.', 'yaymail' ), '<strong>' . YAYMAIL_ADDON_WOO_BOOKING_NAME . '</strong>', '<strong>' . YAYMAIL_ADDON_WOO_BOOKING_NAME . '</strong>' );
		$notice_addon .= '</p> </div>';
		...
		if ( ! YAYMAIL_THIRD_PARTY_WOO_BOOKING_IS_ACTIVE ) {
