...
if ( $order ) :
	...
	<?php if ( 'pending' === $order->get_status() ) : ?>
		...
		echo wp_kses_post( sprintf( __( 'You must pay for this booking before your booking is secured. Please use the following link: %s', 'woocommerce-bookings' ), '<a style="display: block; background: black; padding: 15px 15px; text-align: center; font-size: 20px; margin: 20px 0 0; color: white; text-decoration: none;"href="' . esc_url( $order->get_checkout_payment_url() ) . '">' . __( 'Pay for booking', 'woocommerce-bookings' ) . '</a>' ) );
		?>
		</p>
	<?php endif; ?>
