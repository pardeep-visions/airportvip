...
add_action( 'wp_enqueue_scripts', 'hh_register_additional_styles' );
function hh_register_additional_styles()
{
    wp_register_style( 'themify-icon', get_theme_file_uri('cpts/locations/assets/css/themify.css') );
}
...
function wpse119820_use_different_template_locations( $template ){
   ...
   if( is_singular( 'locations' ) ) {
	...
	if( is_tax('countries')) {
		if( $_template = locate_template( 'cpts/locations/archive-countries.php' ) ){
			//Template found, - use that
			$template = $_template;
		}
	}       
